...
public class SearchActivity extends AppCompatActivity {
    public static final String HEROS_API = "https://www.superheroapi.com/api.php/552714195688026/search/a";
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (Intent.ACTION_SEARCH.equals(intent.getAction())) {
            ...
            ArrayList<Hero> herosListByQuery = JSON.getHerosListByQuery(heroList, query);

            if (herosListByQuery.size() == 0) {
                Toast.makeText(this, getResources().getString(R.string.search_no_results) + query, Toast.LENGTH_SHORT).show();
            }

            // Setup and Handover data to recyclerview
            recyclerView = (RecyclerView) findViewById(R.id.heros_list);
            adapter = new Adapter(SearchActivity.this, herosListByQuery);
    ...
    private class AsyncFetch extends AsyncTask<String, String, ArrayList<Hero>> {
        ...
        protected ArrayList<Hero> doInBackground(String... params) {
            try {
                JSONObject jsonObject = JSON.readJsonFromUrl(HEROS_API);
        ...
        @Override
        protected void onPostExecute(ArrayList<Hero> herosList) {
            //this method will be running on UI thread
            pdLoading.dismiss();

           if(herosList != null) {
               Toast.makeText(SearchActivity.this, getResources().getString(R.string.search_found_entries_from_api) + herosList.size(), Toast.LENGTH_SHORT).show();
           }
        }//onPostExecute
    }//AsyncFetch class
}
